...
import { IconShieldCheck, IconX, IconPhoto, IconAlignLeft, IconLoader2 } from '@tabler/icons-react';
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 14 }}>
          Submit proof for <strong style={{ color: 'var(--text)' }}>{habitName}</strong> to earn +3 leaderboard points
        </div>
        ...
        {tab === 'text' ? (
          <div>
            ...
            <div style={{ fontSize: 10, color: text.length >= 20 ? 'var(--accent-text)' : 'var(--text3)', textAlign: 'right', marginTop: 4 }}>
              {text.length} chars {text.length < 20 && `(${20 - text.length} more needed)`}
            </div>
          </div>
        ) : (
          <div>
        ...
        <button
          ...
          disabled={!canSubmit || loading || (result?.verdict === 'approved')}
          style={{
            width: '100%', padding: 13, borderRadius: 12, border: 'none',
            background: canSubmit && !loading ? 'var(--btn-primary-bg)' : 'var(--bg3)',
            color: canSubmit && !loading ? 'var(--btn-primary-text)' : 'var(--text3)',
