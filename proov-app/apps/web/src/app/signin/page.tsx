...
export default function SignInPage() {
  ...
  // Keeps the connecting overlay up so the sign-in form never flashes underneath it.
  ...
  // render and this would flash the sign-in form before initWeb3Auth() resolves.
  useEffect(() => {
    ...
    initWeb3Auth().then(() => {
      ...
      if (!w.connected) {
        ...
        if (wasOAuthPending) {
          ...
          console.error('[signin] OAuth redirect did not complete', err);
      ...
      setAuthChecking(false);
      if (wasOAuthPending) {
        console.error('[signin] initWeb3Auth failed', err);
        setAuthError(`Sign-in didn't complete${describeError(err)}. Please try again.`);
      }
    });
  ...
  useEffect(() => {
    if (connectError && !isConnected) {
      setConnecting(false);
      console.error('[signin] connect() failed', connectError);
  ...
  const triggerConnect = async (loginProvider = 'google') => {
    ...
    try {
      ...
      sessionStorage.removeItem('proov_oauth_pending');
      setConnecting(false);
      console.error('[signin] connectTo failed', err);
      setAuthError(`Sign-in didn't complete${describeError(err)}. Please try again.`);
      return;
    }
    const c = connectors.find(c => c.id === 'web3auth-aa') ?? connectors[0];
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>
              {phase === 'connecting' && 'Connecting to your account…'}
